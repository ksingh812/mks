...
if(class_exists('kingdomAmazonHelper') != true) {
	class kingdomAmazonHelper extends kingdom 
	{
		...
		public function __construct( $the_plugin=array() ) 
		{
			...
			// ajax actions
			add_action('wp_ajax_kingdomCheckAmzKeys', array( $this, 'check_amazon') );
			add_action('wp_ajax_kingdomImportProduct', array( &$this, 'getProductDataFromAmazon' ));
			
			add_action('wp_ajax_kingdomStressTest', array( &$this, 'stress_test' ));
		}
		...
		public function stress_test()
		{
			...
			if( $action == 'import_images' ){
				...
				if( isset($_SESSION["kingdom_test_product"]) && count($_SESSION["kingdom_test_product"]) > 0 ){
					$product = $_SESSION["kingdom_test_product"];
			...
			if( $action == 'insert_product' ){
				if( isset($_SESSION["kingdom_test_product"]) && count($_SESSION["kingdom_test_product"]) > 0 ){
					...
					if( (int) $insert_id > 0 ){
						...
						$_SESSION["kingdom_test_product"]['local_id'] = $insert_id;
						$return = array( 
							'status' => 'valid',
				...
				if( $asin != "" ){
					...
					if($product['Items']["Request"]["IsValid"] == "True"){
						...
						if (isset($product['Items']['Item']) && count($product['Items']['Item']) > 0) {
							...
							$_SESSION['kingdom_test_product'] = $retProd;
						}
		...
		public function getAmazonCategs()
		{
			...
			$csv_file_content = $this->the_plugin->wp_filesystem->get_contents( $this->the_plugin->cfg['paths']['plugin_dir_path'] . 'assets/browsenodes.csv' );
		...
		public function getProductDataFromAmazon ( $return_amz_data=false )
		{
			...
			$cross_selling = (isset($this->amz_settings["kingdom_amazon"]["cross_selling"]) && $this->amz_settings["kingdom_amazon"]["cross_selling"] == 'yes' ? true : false);
			...
			$product = $aaAmazonWS->responseGroup('Large,ItemAttributes,Offers,Variations,Reviews')->optionalParameters(array('MerchantId' => 'All'))->lookup($asin);
	            ...
	            {           
	                ...
	                { 
						...
						if ( is_object($currentNode) && isset($currentNode->BrowseNodeId) )
	                    	$dmCatSlug = ($currentNode->BrowseNodeId);
		...
		public function productAmazonPriceIsZero( $thisProd ) {
			...
			$price_setup = (isset($this->amz_settings["kingdom_amazon"]["price_setup"]) && $this->amz_settings["kingdom_amazon"]["price_setup"] == 'only_amazon' ? 'only_amazon' : 'amazon_or_sellers');
			...
			if( !isset($offers['ListPrice']) || (int)$offers['ListPrice'] == 0 ) {
		...
		public function productPriceUpdate( $thisProd, $post_id='', $return=true )
		{
			...
			if ( isset($product_meta['product']['sales_price']) )
		...
		public function get_asset_by_id( $asset_id, $inprogress=false, $include_err=false ) {
			...
			$kingdomAssetDownloadCron = new kingdomAssetDownload();
			
			return $kingdomAssetDownloadCron->get_asset_by_id( $asset_id, $inprogress, $include_err );
		}
		...
		public function get_asset_by_postid( $nb_dw, $post_id, $include_variations, $inprogress=false, $include_err=false ) {
			...
			return $kingdomAssetDownloadCron->get_asset_by_postid( $nb_dw, $post_id, $include_variations, $inprogress, $include_err );
		}
		...
		public function get_asset_multiple( $nb_dw='all', $inprogress=false, $include_err=false ) {
			...
			return $kingdomAssetDownloadCron->get_asset_multiple( $nb_dw, $inprogress, $include_err );
		}
