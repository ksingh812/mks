		...
		public function bildThePage ( $options = array(), $alias='', $module=array(), $showForm=true ) 
		{
			...
			foreach ( $options as $theBoxs ) {
				...
				foreach ( $theBoxs as $box_id => $box ){
					...
					// container setup
					$html[] = '<div class="wwcAmzAff-' . ( $box['size'] ) . '">
                        	<div class="wwcAmzAff-' . ( $box['style'] ) . '">';
					...
					if( $box['header'] == true ) {
						$html[] = '<div class="wwcAmzAff-panel-header">
							<span class="wwcAmzAff-panel-title">
							 ...
							 ' . ( $box['toggler'] == true ? '<span class="wwcAmzAff-panel-toggler"></span>' : '' ) . '
						</div>';
					}
						
					$html[] = '<div class="wwcAmzAff-panel-content">';
					if($showForm){
						$html[] = '<form class="wwcAmzAff-form" id="' . ( $box_id ) . '" action="#save_with_ajax">';
					}
					...
					if(count($box['elements']) > 0){
						...
						foreach ( $box['elements'] as $elm_id => $value ){
							...
							if(!in_array( $value['type'], $noRowElements)){
								...
								$html[] = '<div class="wwcAmzAff-form-row' . ($this->tabsElements($box, $elm_id)) . '">
									   <label for="' . ( $elm_id ) . '">' . ( isset($value['title']) ? $value['title'] : '' ) . '</label>
									   <div class="wwcAmzAff-form-item'. ( isset($value['size']) ? " " . $value['size'] : '' ) .'">';
							}
							...
							switch ( $value['type'] ) {
								...
								case 'upload_image':
									...
									if(!empty($val)){
										$imgSrc = $wwcAmzAff->image_resize( $val, $value['thumbSize']['w'], $value['thumbSize']['h'], $value['thumbSize']['zc'] );
								...
								case 'message':
									$html[] = '<div class="wwcAmzAff-message wwcAmzAff-' . ( $value['status'] ) . ' ' . ($this->tabsElements($box, $elm_id)) . '">' . ( $value['html'] ) . '</div>';
								break;
									...
									if(count($value['options']) > 0){
										foreach ($value['options'] as $key => $value){
											$html[] = '<input 
												...
												value="' . ( $value['value'] ) . '" 
												class="wwcAmzAff-button ' . ( $value['color'] ) . ' ' . ( isset($value['pos']) ? $value['pos'] : '' ) . ' ' . ( $value['action'] ) . '" 
											/>';
										}
									}
								...
								case 'multiselect_left2right':
									...
									$html[] = '<div class="wwcAmzAff-multiselect-half wwcAmzAff-multiselect-available" style="margin-right: 2%;">';
									...
									$html[] = '<div class="wwcAmzAff-multiselect-half wwcAmzAff-multiselect-selected">';
									...
									$html[] = '<span style="display: inline-block; width: 24.1%; text-align: center;"><input id="' . esc_attr( $elm_id ) . '-moveright" type="button" value="Move Right" class="moveright wwcAmzAff-button gray"></span>';
									$html[] = '<span style="display: inline-block; width: 24.1%; text-align: center;"><input id="' . esc_attr( $elm_id ) . '-moverightall" type="button" value="Move Right All" class="moverightall wwcAmzAff-button gray"></span>';
									$html[] = '<span style="display: inline-block; width: 24.1%; text-align: center;"><input id="' . esc_attr( $elm_id ) . '-moveleft" type="button" value="Move Left" class="moveleft wwcAmzAff-button gray"></span>';
									$html[] = '<span style="display: inline-block; width: 24.1%; text-align: center;"><input id="' . esc_attr( $elm_id ) . '-moveleftall" type="button" value="Move Left All" class="moveleftall wwcAmzAff-button gray"></span>';
									$html[] = '</div>';
								break;
							...
							if(!in_array( $value['type'], $noRowElements)){
								// close: .wwcAmzAff-form-row
								$html[] = '</div>';
								
								// close: .wwcAmzAff-form-item
								$html[] = '</div>';
							}
					...
					// wwcAmzAff-message use for status message, default it's hidden
					$html[] = '<div class="wwcAmzAff-message" id="wwcAmzAff-status-box" style="display:none;"></div>';
					
					if( $box['buttons'] == true && !is_array($box['buttons']) ) {
						// buttons for each box
						$html[] = '<div class="wwcAmzAff-button-row">
							<input type="reset" value="Reset to default value" class="wwcAmzAff-button gray left" />
							<input type="submit" value="Save the settings" class="wwcAmzAff-button green wwcAmzAff-saveOptions" />
						</div>';
					}
					elseif( is_array($box['buttons']) ){
						...
						foreach ( $box['buttons'] as $key => $value ){
							$html[] = '<input type="submit" value="' . ( $value['value'] ) . '" class="wwcAmzAff-button ' . ( $value['color'] ) . ' ' . ( $value['action'] ) . '" />';
						}
					...
					// close: .wwcAmzAff-panel-content
					$html[] = '</div>';
					
					// close: box style  div (.wwcAmzAff-panel)
					$html[] = '</div>';
...
?>
		<div id="wwcAmzAff-wrapper" class="fluid wrapper-wwcAmzAff">
			...
			<?php
			// show the top menu
			wwcAmzAffAdminMenu::getInstance()->show_menu( $pluginPage );
			?>
				...
				<div class="wwcAmzAff-container clearfix">
					...
					<div id="wwcAmzAff-content-wrap" class="clearfix">
						...
						<div id="wwcAmzAff-content-area">
							<!-- Content Area -->
							<div id="wwcAmzAff-ajax-response"></div>
