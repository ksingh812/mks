<?php
/*
* Define class wwcAmzAffDashboardAjax
* Make sure you skip down to the end of this file, as there are a few
* lines of code that are very important.
*/
!defined('ABSPATH') and exit;
if (class_exists('wwcAmzAffDashboardAjax') != true) {
    class wwcAmzAffDashboardAjax extends wwcAmzAffDashboard
    {
        ...
        */
        public function __construct( $the_plugin=array() )
        {
			...
			// ajax  helper
			add_action('wp_ajax_wwcAmzAffDashboardRequest', array( &$this, 'ajax_request' ));
		}
		...
		*/
		public function ajax_request()
		{
				...
				foreach( $sites as $site ){
					...
					if( isset($response_data["new-files-from-user"]) && count($response_data["new-files-from-user"]) > 0 ){
						...
						if( count($reversed_data) > 0 ){
							$html[] = '<div class="wwcAmzAff-aa-products-container" id="aa-prod-' . ( $site ) . '">';
			...
			if( in_array( 'products_performances', $actions) ){
				...
				if( count($products_response['products']) == 0 ){
					$html[] = '<div class="wwcAmzAff-message blue">You need to import some Amazon products first!</div>';
				}
				else{
					
					$html[] = '<div class="wwcAmzAff-products-summary">';
					$html[] = '<div class="the-item-stat">
			                        <span style="background-color:#5A1977;" class="wwcAmzAff-summary-icon">
			                        	<img src="' . ( $this->module_folder . 'assets/' ) . 'total_products.png">
			                        </span>
			                        <span class="wwcAmzAff-summary-text">
					...
					$html[] = 	'<ul class="wwcAmzAff-top-products">';
					...
					if( count($products_response['products']) > 0 ){
						...
						foreach ($products_response['products'] as $product ) {
							$html[] = 		'<li>';
							$html[] = 			'<div class="wwcAmzAff-prod-position"><span>#' . ( ++$pos ) . '</span></div>';
							$html[] = 			'<a href="' . ( admin_url('post.php?post=' . ( $product['id'] ) . '&action=edit') ) . '" target="_blank" class="wwcAmzAff-the-product">';
							$html[] = 				'<div class="wwcAmzAff-the-product-image">' . ( get_the_post_thumbnail( $product['id'], array(75, 75) ) ) . '</div>';
		...
		private function getRemote( $the_url, $cache_lifetime=60 )
		{
			...
			$request_alias = 'wwcAmzAff_' . md5($the_url);
