...
wwcAmzAffInsaneMode = (function($) {
	...
	(function init() {
		...
		$(document).ready(function() {
			...
			background_loading_container = maincontainer.find(".wwcAmzAff-insane-work-in-progress");
			containers = {
			    loadprods:      {
			        wrap         : maincontainer.find('#wwcAmzAff-content-scroll'),
			        search       : maincontainer.find('#wwcAmzAff-content-search'),
			        grab         : maincontainer.find('#wwcAmzAff-content-grab'),
			        bulk         : maincontainer.find('#wwcAmzAff-content-bulk'),
			        status       : maincontainer.find('#wwcAmzAff-loadprods-status')
			    },
			    loadstatus:     {
			        wrap         : maincontainer.find('#wwcAmzAff-insane-loadstatus')
			    },
			    queueprods:     {
			        wrap         : maincontainer.find('#wwcAmzAff-queued-products'),
			        results      : maincontainer.find('#wwcAmzAff-queued-results-stats'),
			        export       : maincontainer.find('#wwcAmzAff-export-asins'),
			        prods        : null,
			        check_all    : maincontainer.find('.wwcAmzAff-check-all')
			    },
			    importprods:    {
			        wrap         : maincontainer.find('#wwcAmzAff-insane-import-parameters'),
			        estimate     : null,
			        time         : null,
                    logo         : null,
                    screen_tmp   : maincontainer.find('#wwcAmzAff-import-screen .wwcAmzAff-iip-lightbox')
			    },
                importstatus:   {
                    wrap         : maincontainer.find('#wwcAmzAff-insane-importstatus')
                },
			};
			containers.queueprods.prods      = containers.queueprods.wrap.find('.WZC-products-scroll-cointainer');
			containers.importprods.estimate  = containers.importprods.wrap.find('.wwcAmzAff-insane-import-estimate');
			containers.importprods.time      = containers.importprods.estimate.find('.wwcAmzAff-insane-import-ETA');
            containers.importprods.logo      = containers.importprods.estimate.find('.wwcAmzAff-insane-import-ETA-logo');
            ...
            default_import_settings = maincontainer.find('#wwcAmzAff-import-settings').html();
	...
	{
		var speedometer       = maincontainer.find("#wwcAmzAff-speedometer"),
		...
		// products per minute
		speedometer.find('#wwcAmzAff-speedometer-name i').html( new_value );
    ...
    var queueprod = (function() {
        ...
        function set_queue_width() {
            ...
            var list = containers.queueprods.prods.find("ul"),
                status = containers.queueprods.wrap.hasClass('wwcAmzAff-open') ? 'open' : 'close';
            if ( status == 'close' ) {
        ...
        function create_products_scroll( status ){
            containers.queueprods.prods.width( $("div#wwcAmzAff-content").width() - 70 );
            set_queue_width();
        ...
        {
            var list = containers.queueprods.prods.find("ul"),
                btn = maincontainer.find("#wwcAmzAff-expand-all");
            var is_init = is_init || false;
            
            // toggle box status
            if ( status == 'toggle' ) {
                status = containers.queueprods.wrap.hasClass('wwcAmzAff-open') ? 'close' : 'open';
            }
    
            if ( status == 'close' ) {
                //console.log( 'close' );
                list.width( containers.queueprods.wrap.data("list_width") );
                containers.queueprods.wrap.removeClass('wwcAmzAff-open').addClass('wwcAmzAff-close');
                ...
                list.width( "100%" );
                containers.queueprods.wrap.removeClass('wwcAmzAff-close').addClass('wwcAmzAff-open');
	...
	function triggers()
	{
		...
		maincontainer.on('click', "#wwcAmzAff-expand-all", function(e){
			e.preventDefault();
			
			var that     = $(this),
                parent   = that.parents().eq(2); 
 
			parent.find('.wwcAmzAff-insane-panel-headline a[href="#wwcAmzAff-queued-products"]').trigger('click');
		...
		// TABS
		maincontainer.find(".wwcAmzAff-insane-tabs").each(function(){
			var that = $(this),
				btns = that.find(".wwcAmzAff-insane-panel-headline a"),
				tabs = that.find(".wwcAmzAff-insane-tab-content");
			
			that.on('click', '.wwcAmzAff-insane-panel-headline a', function(e){
				e.preventDefault();
			...
			//$(".wwcAmzAff-insane-panel-headline a").eq(2).click();
		});
        ...
        maincontainer.on('click', ".wwcAmzAff-insane-buton-logs", function(e){
            e.preventDefault();
            
            var that    = $(this),
                log     = that.data('logcontainer');
            
            $('.wwcAmzAff-insane-container-logs').filter(function(i){
                return $(this).prop('id') == log;
            }).toggle('wwcAmzAff-logs-open');
        });
	}
        ...
        if( status == 'show' ){
            if( row.size() > 0 ){
                if( row.find('.wwcAmzAff-row-loading-marker').size() == 0 ){
                    ...
                    var html = '<div class="wwcAmzAff-meter wwcAmzAff-animate" style="width:30%; margin: 10px auto;"><span style="width:100%"></span></div><div class="wwcAmzAff-loading-text">' + lang.loading + '</div>';
                    //if ( isextra ) {
    ...
    function set_status_msg_generic( status, msg, op, from ) {
        ...
        var html = '\
            <li class="wwcAmzAff-log-' + wrap.li + '">\
                <i class="fa fa-' + wrap.i + '"></i>\
                <span class="wwcAmzAff-insane-logs-frame">' + misc.get_current_date() + '</span>\
                <span class="wwcAmzAff-insane-logs-frame">' + _op + '</span>\
                <br />\
                <span class="wwcAmzAff-insane-logs-msg"> ' + msg + '</span>\
            </li>',
            html_ = '\
            <span class="wwcAmzAff-message wwcAmzAff-' + wrap.span + '">\
                <span class="wwcAmzAff-insane-logs-frame">' + misc.get_current_date() + '</span>\
                <span class="wwcAmzAff-insane-logs-frame">' + _op + '</span>\
                <br />\
                <span class="wwcAmzAff-insane-logs-msg"> ' + msg + '</span>\
        ...
        if ( from == 'loadprod' ) {
            ...
            containers.loadstatus.wrap.find('ul.wwcAmzAff-insane-logs').prepend( html );
        } else {
            containers.importstatus.wrap.find('ul.wwcAmzAff-insane-logs').prepend( html );
        }
    };
    ...
    var loadprod = (function() {
        ...
        function triggers() {
            ...
            // BULK LOAD ASINs
            box.on('click', 'form#wwcAmzAff-import-products #wwcAmzAff-addASINtoQueue', function(e) {
                e.preventDefault();
            ...
            // GRAB ASINs
            box.on('click', 'form#wwcAmzAff-grab-products #wwcAmzAff-grabb-button', function(e) {
                e.preventDefault();
            ...
            // SEARCH PRODS
            box.on('submit', 'form#wwcAmzAff-search-products', function(e){
            //box.on('click', 'form#wwcAmzAff-search-products #wwcAmzAff-button', function(e) {
                e.preventDefault();
            ...
            // export asins
            containers.queueprods.export.on('click', 'form#wwcAmzAff-export-form #wwcAmzAff-export-button', function(e) {
                e.preventDefault();
            ...
            var search_wrap         = containers.loadprods.search,
                categ_wrap          = search_wrap.find('#wwcAmzAff-select-on-category');

            // change category
            search_wrap.on( 'change', 'select#wwcAmzAff-search-category', function(){
                var that = $(this);
            ...
            search_wrap.on( 'change', '.wwcAmzAff-param-optional.wwcAmzAff-param-node select', function(){
                var that = $(this);
            ...
            search_wrap.on('change', 'select[name="wwcAmzAff-search[Sort]"]', function (e) {
                var that    = $(this);
                sort_tooltip( that );
            });
        };
        ...
        function bulk_add_asin_to_queue() {
            ...
            var box = containers.loadprods.bulk,
                asins_str = $.trim( box.find('#wwcAmzAff-content-bulk-asin').val() ),
                delimiter = box.find("input[name=wwcAmzAff-csv-delimiter]:checked").attr('id').split('radio-'),
        ...
        function grab_parse_url() {
            ...
            var box = containers.loadprods.grab,
                form = box.find('form#wwcAmzAff-grab-products');

            var data = {
                'action'        : 'wwcAmzAffIM_LoadProdsGrabParseURL',
        ...
        function search_prods( form ) {
            ...
            var box             = containers.loadprods.search,
                form            = typeof form != 'undefined' ? form : box.find('form#wwcAmzAff-search-products'),
            ...
            data.push(
                ...
                {name: 'action',            value: 'wwcAmzAffIM_LoadProdsBySearch'},
            ...
            var data            = {
                'debug_level'       : debug_level,
                'action'            : 'wwcAmzAffIM_LoadProdsBySearch',
                'operation'         : 'search',
        ...
        function load_prods_by_asin( asin, op ) {
            ...
            function do_load() {
                ...
                var data = {
                    'debug_level'       : debug_level,
                    'action'            : 'wwcAmzAffIM_LoadProdsByASIN',
                    'operation'         : op,
                    'operation_id'      : operation_id,
                    'asins'             : asin_step,
                    'page'              : contor + 1,
        ...
        function set_results( asin, type, response ) {
            ...
            if ( containers.queueprods.results.find('.wwcAmzAff-stats-' + type + ' span span').length ) {
                containers.queueprods.results.find('.wwcAmzAff-stats-' + type + ' span span').html( asins[type].length );
            }
            //console.dir( asins );
        };
        ...
        function export_asins( form ) {
            ...
            var box = containers.queueprods.export,
                form = typeof form != 'undefined' ? form : box.find('form#wwcAmzAff-search-products'),
                delimiter = box.find("input[name=wwcAmzAff-export-delimiter]:checked").attr('id').split('radio-'),
                delimiter = delimiter[1],
                export_asins_type = box.find("#wwcAmzAff-export-asins-type").val(),
                _asins = [];
        ...
        function search_select_pages( category ) {
            var category    = $.trim( category ),
                ...
                dropdown    = container.find('#wwcAmzAff-search-nbpages'),
        ...
        function select_category() {
            var search_on   = maincontainer.find("#wwcAmzAff-search-search_on"),
                def_search  = search_on.val(),
                custom      = maincontainer.find("#wwcAmzAff-node"),
                dropdown    = maincontainer.find("#wwcAmzAff-search-category");
        ...
        function keyword_autocomplete() {
            var autocomplete = maincontainer.find(".wwcAmzAff-search-completion");
            
            maincontainer.on('keyup', "input#wwcAmzAff-search-keyword", function(){
                
                var that = $(this),
                    data = {
                        'action': 'wwcAmzAffIM_KeywordAutocomplete',
                        'keyword': that.val()
                    };
            ...
            autocomplete.on('click', 'li', function(){
                ...
                maincontainer.find("input#wwcAmzAff-search-keyword").val( text );
                autocomplete.html("");
                autocomplete.hide(); 
            });
        ...
        function get_category_params( category ) {
            ...
            var box             = containers.loadprods.search,
                categ_wrap      = box.find('#wwcAmzAff-select-on-category'),
                dropdown        = categ_wrap.find("#wwcAmzAff-search-category"),
                category        = category || dropdown;

            var data = {
                'action'        : 'wwcAmzAffIM_getCategoryParams',
            ...
            $.post(ajaxurl, data, function(response) {
                ...
                if (response.status == 'valid') {
                    ...
                    box.find('li.wwcAmzAff-param-optional').remove(); //detach()
                    ...
                    (function(){
                        var search_wrap     = containers.loadprods.search,
                            that            = search_wrap.find('select[name="wwcAmzAff-search[Sort]"]');
                        sort_tooltip( that );
                    })();
                }
                loading( 'close' );
        ...
        function get_browse_nodes( that, category ) {
            ...
            var box             = containers.loadprods.search,
                ...
                len             = box.find('.wwcAmzAff-param-optional.wwcAmzAff-param-node').length; // prev element value
            ...
            var next = null;
            while( (next = parent_li.next('.wwcAmzAff-param-optional.wwcAmzAff-param-node')).length > 0 ) {
                next.remove();
            }
            ...
            if( ascensor_value != "" ){
                var data = {
                    'action'        : 'wwcAmzAffIM_getBrowseNodes',
                    'category'      : category.find('option:selected').val(),
        ...
        function is_empty_queue( show ) {
            var show        = show || true,
                msgbox      = containers.queueprods.wrap.find('#wwcAmzAff-queued-message'),
    ...
    var importprod = (function() {
        ...
        function triggers() {
            // import products
            wrap.on('click', '#wwcAmzAff-import-products-button', function(e) {
                e.preventDefault();
            ...
            // stop import
            $('body').on('click', '#wwcAmzAff-import-stop-button', function(e) {
                e.preventDefault();
        ...
        function import_products() {
            ...
            function do_import( is_init ) {
                ...
                if ( $.inArray(process_status, ['stop', 'finished']) > -1 ) {
                    screen.find('.wwcAmzAff-iip-tail ul.WZC-keyword-attached').html('');
                    show_process_status( screen, process_status );
                    heartbeat.stop();
                    return false;
                }
                ...
                // REAL AJAX IMPORT
                    ...
                    data.push(
                        {name: 'debug_level',       value: debug_level},
                        {name: 'action',            value: 'wwcAmzAffIM_ImportProduct'},
                        ...
                        if (response.status == 'valid') {
                            ...
                            if ( misc.hasOwnProperty(response, 'show_download_lightbox')
                                ...
                                if ( 1 ) {
                                    ...
                                    wwcAmzAffAssetDownload.download_asset( 
                                        $('.wwcAmzAff-images-tail').find('li').eq(0), undefined, 100, function() {
        ...
        function get_current_next( pms ) {
            ...
            if ( show ) {
                ...
                var ul = box_screen.find('.wwcAmzAff-iip-tail ul.WZC-keyword-attached');
                if ( current.length ) {
                    current.removeClass('selected').addClass('imported')
                        .find('.wwcAmzAff-checked-product').before(
                            '<div class="WZC-product-current">' + lang.current_product_title + '</div>'
                        );
                    current.find('.wwcAmzAff-checked-product input').prop('disabled', true);
                    ul.html( current );
                }
                if ( next.length ) {
                    ...
                    next.find('.wwcAmzAff-checked-product input').prop('disabled', true);
                    ul.append( next );
                }
            }
            return {
        ...
        function show_progress( pms ) {
            ...
            var im = {
                'prods'         : box_screen.find('.wwcAmzAff-iip-process-progress-bar.im-products'),
                'images'        : box_screen.find('.wwcAmzAff-iip-process-progress-bar.im-images'),
                'variations'    : box_screen.find('.wwcAmzAff-iip-process-progress-bar.im-variations')
            };
            ...
            for (var i in im) {
                if ( misc.hasOwnProperty( im, i ) ) {
                    ...
                    if ( __im.length ) {
                        var im_text     = __im.find('.wwcAmzAff-iip-process-progress-text'),
                            im_marker   = __im.find('.wwcAmzAff-iip-process-progress-marker');
        ...
        function show_estimate( pms ) {
            var pms         = typeof pms == 'object' ? pms : {},
                ...
                cssClass    = 'wwcAmzAff-insane-logo-level' + level,
            ...
            containers.importprods.logo
                .removeClass('wwcAmzAff-insane-logo-level' + level_prev)
                .addClass('wwcAmzAff-insane-logo-level' + level);
        }
        ...
        function show_estimate_screen( pms ) {
            ...
            //if ( speed > 0 ) {
                ...
                box_screen.find('#wwcAmzAff-iip-estimate-speed').html( speed_text );
            //}

            // estimated time text
            time_txt = $.trim( time_txt );
            box_screen.find('#wwcAmzAff-iip-estimate-time span').html( time_txt );

            if ( time_txt == '' ) {
                //box_screen.find('#wwcAmzAff-iip-estimate-time span').html( '--' );
        ...
        function show_process_status( box_screen, status ) {
            ...
            box_screen.find('#wwcAmzAff-iip-estimate-status span').html( status_html );
            if ( $.inArray(status, ['stop', 'finished', 'stop_']) > -1 ) {
                //box_screen.find('#wwcAmzAff-iip-estimate-status input').addClass('gray').prop('disabled', true);
                box_screen.find('#wwcAmzAff-iip-estimate-status input')
                    .data('is_close', true)
        ...
        function loading( status, msg, from ) {
            ...
            if ( from == 'external' ) {
                ...
                __screen = big_parent.find('#wwcAmzAff-iip-screen');
                if ( __screen.length ) {
                    __screen.remove();
                    screen = null;
                }
                ...
                if ( status == 'show' ) {
                    ...
                    __screen.prop('id', 'wwcAmzAff-iip-screen');
                    
                    // import images
                    var found = find_parameter( import_params, 'import_type' );
                    if ( current_prods_data.nb_images <= 0 || ( found.status && found.value != 'default' ) ) {
                        __screen.find('.wwcAmzAff-iip-process-progress-bar.im-images').remove();
                    }
                    // import variations
                    if ( current_prods_data.nb_variations <= 0 ) {
                        __screen.find('.wwcAmzAff-iip-process-progress-bar.im-variations').remove();
                    }
                    
                    big_parent.prepend( __screen );
                    screen = big_parent.find('#wwcAmzAff-iip-screen');
                    screen.css({
    ...
    var heartbeat = (function() {
        ...
        function doit() {
            function _doit() {
                ...
                if ( inner_method == 'ajax' ) {
                    ...
                    data.push(
                        {name: 'debug_level',       value: debug_level},
                        {name: 'action',            value: 'wwcAmzAffIM_InsaneAjax'},
                    ...
                    var resp_msg = cookies.read('wwcAmzAff_opStatusMsg');
        ...
        function show_response( status, msg ) {
            ...
            if ( for_module == 'load' ) {
                container   = containers.loadprods.wrap.parent().parent();
                loader      = container.find('.wwcAmzAff-row-loading-marker');
                loader_txt  = loader.find('div.wwcAmzAff-loading-text');
            
            } else if ( for_module == 'import' ) {
                container   = containers.importprods.wrap.parent().parent();
                loader      = container.find('#wwcAmzAff-iip-screen');
                loader_txt  = loader.find('.wwcAmzAff-iip-log');
            }
