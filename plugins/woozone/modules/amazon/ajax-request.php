...
add_action('wp_ajax_wwcAmzAff_load_product', 'wwcAmzAff_load_product_callback');
function wwcAmzAff_load_product_callback() {
	global $wwcAmzAff;
	...
	if($product['Items']["Request"]["IsValid"] == "True"){
		...
		if(count($product['Items']['Item']) > 0){
			...
			$prod_id = $wwcAmzAff->addNewWooProduct($retProd);
...
add_action('wp_ajax_amazon_request', 'wwcAmzAffamazon_request_callback');
function wwcAmzAffamazon_request_callback() {
	global $wwcAmzAff;
	...
	?>
		...
		<?php
			...
			if(count($response['Items']['Item']) > 0){
				echo '
				<div class="wwcAmzAff-product-box">
					<table class="product">
				';
				$cc = 0;
						...
						<td class="product-options">
							<?php
								if($value['CustomerReviews']['HasReviews'] == true){
									echo '<a class="thickbox wwcAmzAff-option-btn" href="' . ( $value['CustomerReviews']['IFrameURL'] ) . '&TB_iframe=true"> <img src="'. ( $plugin_uri ) .'assets/comments.png" /> Customer Reviews</a>';
								}
							?>
