<?php
/*
* Define class wwcAmzAffDashboard
* Make sure you skip down to the end of this file, as there are a few
* lines of code that are very important.
*/
!defined('ABSPATH') and exit;
if (class_exists('wwcAmzAffDashboard') != true) {
    class wwcAmzAffDashboard
    {
        /*
        ...
        */
        ...
        { 
			...
			new wwcAmzAffDashboardAjax( $this->the_plugin );
			...
			$this->addBox( 'products_performances', 'Top 
				<select class="wwcAmzAff-numer-items-in-top">
	    ...
	    */
		...
		{
			wp_register_style( 'wwcAmzAff-DashboardBoxes', $this->module_folder . 'app.css', false, '1.0' );
        	wp_enqueue_style( 'wwcAmzAff-DashboardBoxes' );
		}
		
		public function admin_load_scripts()
		{
			wp_enqueue_script( 'wwcAmzAff-DashboardBoxes', $this->module_folder . 'app.class.js', array(), '1.0', true );
		}
		...
		{ 
			...
			if( isset($id) && trim($id) != "" && !isset($this->boxes[$id]) ){
				...
				$box[] = '<div class="wwcAmzAff-dashboard-status-box">';
		...
		public function formatRow( $content=array() )
		{
			...
			$html[] = '<div class="wwcAmzAff-dashboard-status-box-row">';
			...
			if( isset($content['ajax_content']) && $content['ajax_content'] == true ){
				$html[] = '<div class="wwcAmzAff-dashboard-status-box-content is_ajax_content">';
		...
		public function aateam_products()
		{
			...
			$html[] = '<ul class="wwcAmzAff-aa-products-tabs">';
			$html[] = 	'<li class="on">';
			$html[] = 		'<a href="javascript: void(0)" class="wwcAmzAff-aa-items-codecanyon">CodeCanyon</a>';
			$html[] = 	'</li>';
			$html[] = 	'<li>';
			$html[] = 		'<a href="javascript: void(0)" class="wwcAmzAff-aa-items-themeforest">ThemeForest</a>';
			$html[] = 	'</li>';
			$html[] = 	'<li>';
			$html[] = 		'<a href="javascript: void(0)" class="wwcAmzAff-aa-items-graphicriver">GraphicRiver</a>';
			$html[] = 	'</li>';
			$html[] = '</ul>';
		...
		{
			$html = array();
			$html[] = '<div class="wwcAmzAff-audience-graph" id="wwcAmzAff-audience-visits-graph" data-fromdate="' . ( date('Y-m-d', strtotime("-1 week")) ) . '" data-todate="' . ( date('Y-m-d') ) . '"></div>';
		...
		{
			$html = array();
			$html[] = '<div class="wwcAmzAff-website-preview">';
			$html[] = 	'<h4><b style="color:#a46497;">WooZone</b> is a plugin that allows you to import Amazon products in your WooCommerce store in no time!</h4>';
			...
			$html[] = 	'<h4>Thank you for buying: <a target="_blank" href="http://codecanyon.net/item/plugin/' . ( get_option('wwcAmzAff_register_item_id') ) . '">' . ( get_option('wwcAmzAff_register_item_name') ) . '</a></h4>';
			$html[] = 	'<h4>Licence: <a target="_blank" href="http://codecanyon.net/licenses/regular_extended">' . ( get_option('wwcAmzAff_register_licence') ) . '</a></h4>';
			$html[] = '</div>';
		...
		{
			$html = array();
			$html[] = '<ul class="wwcAmzAff-summary-links">';
...
// Initialize the wwcAmzAffDashboard class
//$wwcAmzAffDashboard = wwcAmzAffDashboard::getInstance( isset($module) ? $module : array() );
//$wwcAmzAffDashboard = new wwcAmzAffDashboard( isset($module) ? $module : array() );
// $wwcAmzAff->cfg, ( isset($module) ? $module : array()) 
