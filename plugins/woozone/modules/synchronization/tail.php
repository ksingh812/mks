<?php
/*
* Define class wwcAmzAffTailSyncMonitor
* Make sure you skip down to the end of this file, as there are a few
* lines of code that are very important.
*/
!defined('ABSPATH') and exit;
if (class_exists('wwcAmzAffTailSyncMonitor') != true) {
    class wwcAmzAffTailSyncMonitor
    {
        /*
        ...
        */
        public function __construct()
        {
            ...
            // ajax helper
            add_action('wp_ajax_wwcAmzAffSyncAjax', array( &$this, 'ajax_request' ));
        }
        ...
        * Singleton pattern
        *
        * @return wwcAmzAffTailSyncMonitor Singleton instance
        */
...
?>
        ...
        <link rel='stylesheet' href='<?php echo $this->module_folder;?>assets/sync-log.css' type='text/css' media='all' />
                            ...
                            <div class="wwcAmzAff-grid_4">
                                <div class="wwcAmzAff-panel">
                                    ...
                                    <div id="wwcAmzAff-sync-log" class="wwcAmzAff-panel-content" data-module="<?php echo $module; ?>">
                                        ...
                                        <?php
                                           $lang = array(
                                               'no_products'          => __('No products available.', 'wwcAmzAff'),
                                               'sync_now'             => __('Sync now (<span>{nb}</span> remained)', 'wwcAmzAff'),
                                               'sync_now_msgformat'   => __('(ASIN: {1} / ID: #{2}): {3}', 'wwcAmzAff'),
                                               'sync_now_finished'    => __('Sync now is finished.', 'wwcAmzAff'),
                                               'sync_now_inwork'      => __('Sync all now in progress. Please be patient till it\'s finished.', 'wwcAmzAff'),
                                               'sync_now_stop_btn'    => __('stop processing.', 'wwcAmzAff'),
                                               'sync_now_stopped'     => __('Sync now is stopped.', 'wwcAmzAff'),
                                        ...
                                        <?php if ( $module == 'synchronization' ) { ?>
                                            <div class="wwcAmzAff-sync-stats">
                                        ...
                                        <?php } else { ?>
                                        <div class="wwcAmzAff-sync-info wwcAmzAff-box-stats">
											  <?php echo $this->get_pagination(array(
                                              		'position' 		=> 'top',
                                              		'with_wrapp' 	=> true
                                              )); ?>
                                        ...
                                        <div class="wwcAmzAff-sync-table <?php echo ( $module == 'synchronization' ? 'synchronization' : 'stats_prod' ); ?>">
                                          <table cellspacing="0">
                                            <thead>
...
<?php
        ...
        private function sync_settings( $pms=array() ) {
            ...
            $ss = self::$settings;
?>
                                        <form class="wwcAmzAff-sync-settings">
                                          ...
                                          <p>
                                            ...
                                            <select id="sync_recurrence" name="sync_recurrence">
                                            <?php
                                            ...
                                            <select id="sync_products_per_request" name="sync_products_per_request">
                                            <?php
...
<?php
        ...
        private function sync_stats() {
            ...
            // last report
            $report_last_date = get_option('wwcAmzAff_report_last_date', false);

            // last sync cycle
            $recurrence = $ss['sync_recurrence'];
            $recurrence2 = self::$sync_recurrence["$recurrence"];
            $sync_start_time = get_option('wwcAmzAff_sync_first_updated_date', false);
            
            $sync_duration = get_option('wwcAmzAff_sync_cycle_stats', array());
            $sync_duration2 = 0;
            ...
            $sync_currentlist_last_product = get_option('wwcAmzAff_sync_currentlist_last_product', 0);
            $sync_last_updated_product = get_option('wwcAmzAff_sync_last_updated_product', 0);
            ...
            $sync_currentlist_nb_products = get_option('wwcAmzAff_sync_currentlist_nb_products', 0);
            ...
            if ( !empty($sync_currentlist_nb_products) && !empty($sync_products_per_request) ) {
                ...
                // 2 minutes * 60 seconds per minute - wwcAmzAff_sync_products
                ...
                <tbody>
                    <tr>
                        <td width="70%">
                            ...
                            <span class="wwcAmzAff-message <?php echo $sync_status == 1 ? 'wwcAmzAff-success' : 'wwcAmzAff-info'; ?>"><?php echo $sync_status_text; ?></span>
                            <ul>
                                <li>
                                    <?php _e('Recurrence', $this->the_plugin->localizationName);?>:
...
<?php
        ...
        private function get_products( $pms=array() ) {
            ...
            $last_updated_product = (int) get_option('wwcAmzAff_sync_last_updated_product', true);
        ...
        private function row_view_html( $row, $is_child=false ) {
            ...
            $text_hits = '<i class="wwcAmzAff-prod-stats-number hits">' . ( $row['stats_hits'] ) . '</i>';
            $text_added_to_cart = '<i class="wwcAmzAff-prod-stats-number add-to-cart">' . ( $row['stats_added_to_cart'] ) . '</i>';
            $text_redirected_to_amazon = '<i class="wwcAmzAff-prod-stats-number redirect-to-amazon">' . ( $row['stats_redirected_to_amazon'] ) . '</i>';
            }
            
            if ( $row['module'] == 'synchronization' ) {
            $ret = '
                    <tr class="wwcAmzAff-sync-table-row' . $tr_css . '" data-id=' . $row['id'] . ' data-asin=' . $row['asin'] . $data_parent_id . '>
                        <td><i class="fa fa-flag" title="' . $row['sync_status']['text'] . '"></i></td>
                        <td><img src="' . $row['thumb'] . '" alt="' . $row['title'] . '" /> ' . $text_asin . $row['asin'] . ' / ' . $text_id . $row['id'] . $childs_btn . '</td>
                        ...
                        <td>' . $row['sync_data'] . '</td>
                        <td class="wwcAmzAff-sync-now"><button>' . $text_sync_now . '</button></td>
                    </tr>
                ';
            } else {
            $ret = '
                    <tr class="wwcAmzAff-sync-table-row' . $tr_css . '" data-id=' . $row['id'] . ' data-asin=' . $row['asin'] . $data_parent_id . '>
                        ...
                        <td><img src="' . $row['thumb'] . '" alt="' . $row['title'] . '" /> ' . $text_asin . $row['asin'] . $childs_btn . '</td>
                        <td><a href="' . $row['link_edit'] . '" target="_blank">' . $row['title'] . '</a></td>
                        <td>' . $text_hits . '</td>
        ...
        private function get_thumb_src_default() {
            return $this->module_folder . 'assets/prod-thumb-default.png';
        }
        ...
        public function ajax_request()
        {
            ...
            if ( $action == 'save_settings' ) {
                ...
                $ret = array_merge($ret, array(
                    'status'    => 'valid',
                    'msg'       => '<div class="wwcAmzAff-sync-settings-msg wwcAmzAff-message wwcAmzAff-success">' . __('Sync settings saved successfully.', $this->the_plugin->localizationName) . '</div>',
				...
				$_SESSION['wwcAmzAff_sync']['paged'] = $new_paged;
				...
				if ( 'post_per_page' == $action ) {
					...
					if ( $new_post_per_page == 'all' ){
						$_SESSION['wwcAmzAff_sync']['posts_per_page'] = 'all';
					}
					else if ( (int)$new_post_per_page == 0 ){
						$max_prods = $this->get_interface_max_products(true);
						$_SESSION['wwcAmzAff_sync']['posts_per_page'] = $max_prods;
					}
					else {
						$_SESSION['wwcAmzAff_sync']['posts_per_page'] = (int) $new_post_per_page;
					}
	
					// reset the paged as well
					$_SESSION['wwcAmzAff_sync']['paged'] = 1;
				}
				else {
					$new_paged = $paged < 1 ? 1 : $paged;
					$_SESSION['wwcAmzAff_sync']['paged'] = $new_paged;
		...
		private function build_pagination_vars() {
			$ses = isset($_SESSION['wwcAmzAff_sync']) ? $_SESSION['wwcAmzAff_sync'] : array();
			$max_prods = $this->get_interface_max_products(true);
		...
		private function get_pagination( $pms=array() )
		{
			...
			{
				if ($with_wrapp) {
					$css_pag = 'wwcAmzAff-sync-pagination ';
					$css_pag .= ($position == 'bottom' ? 'wwcAmzAff-sync-top' : 'wwcAmzAff-sync-bottom');
				...
				$html[] = 				'</option>';
				$html[] =			'</select>';
				$html[] = 			'<label for="wwcAmzAff-post-per-page" style="width:57px">' . __('per page', $this->the_plugin->localizationName) . '</label>';
				$html[] = 		'</div>';
...
//$wwcAmzAffTailSyncMonitor = new wwcAmzAffTailSyncMonitor($this->cfg, $module);
$wwcAmzAffTailSyncMonitor = wwcAmzAffTailSyncMonitor::getInstance();