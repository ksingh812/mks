...
class wwcAmazonSyncronize
{
	 ...
	 */
	public function __construct($wwcAmzAff)
	{
		global $wpdb;
		...
		$this->cfg['available_setup'] = (array) get_option($this->alias . '_sync');
        $this->updateSyncRules();
        
        // ajax  helper
        //add_action('wp_ajax_wwcAmzAffSyncProd', array( &$this, 'ajax_request' ));
	}
	...
	public function updateTheProduct( $asins=array(), $return='die' )
	{
		...
		//$plugin_uri = $this->the_plugin->cfg['paths']['plugin_dir_url'] . 'modules/amazon/';
		...
		*/
		$aaAmazonWS = $this->the_plugin->amzHelper->aaAmazonWS;

		// create request by ASIN
		// Large,ItemAttributes,Offers
		$aaAmazonWS->responseGroup('Large,ItemAttributes,OfferFull,Variations,Reviews,PromotionSummary,SalesRank');
		$aaAmazonWS->optionalParameters( array('MerchantId' => 'All') );
		$products = $aaAmazonWS->lookup( implode(",", $asins) );
        ...
        //if($products['Items']["Request"]["IsValid"] != "True"){
            if (1) {
                ...
                if ( count($asins_notfound) > 0 ) {
                    ...
                    $__sync_prod_notfound = get_option('wwcAmzAff_sync_prod_notfound', true);
                    ...
                    update_option('wwcAmzAff_sync_prod_notfound', $__sync_prod_notfound);
              
                    foreach ($asins_notfound as $localID => $asin) {
                        $this->trash_post( $localID ); // product & product variations if it's the case!
                    ...
                    // update wwcAmzAff_remaining_at, take this as marker for features sync
                    $last_one = array_keys($asins);
                    $last_one = end($last_one);
                    if ( $is_from_cron ) {
                        update_option('wwcAmzAff_sync_last_updated_product', $last_one);
                    }
                }
		...
		if(1){
			...
			*/
            ...
            if ( empty($arrProds) ) {
                // update wwcAmzAff_remaining_at, take this as marker for features sync
                if ( $is_from_cron ) {
                    update_option('wwcAmzAff_sync_last_updated_product', end(array_keys($asins)));
                }
            }
			...
			foreach ($arrProds as $thisProd) { // products loop
				...
				if(count($thisProd) > 0) { // product is amazon valid
					...
					*/
                    ...
                    // update wwcAmzAff_remaining_at, take this as marker for features sync
                    if ( $is_from_cron ) {
                        update_option('wwcAmzAff_sync_last_updated_product', $localID);
                    }
                    
                    // new cycle => first product updated date
                    if ( $is_from_cron ) {
                        $first_updated_date = get_option('wwcAmzAff_sync_first_updated_date', '');
                        $last_updated_product = get_option('wwcAmzAff_sync_last_updated_product', 0);
                        if ( empty($last_updated_product) ) {
                            update_option('wwcAmzAff_sync_first_updated_date', $this->the_plugin->last_update_date());
                        }
                    }
    ...
    private function select_products() {
        ...
        $last_updated_product = (int) get_option('wwcAmzAff_sync_last_updated_product', 0);
    ...
    public function cron_full_cycle( $pms, $return='die' ) {
        ...
        $first_updated_date = (int) get_option('wwcAmzAff_sync_first_updated_date', 0);
        $recurrence = (int) ( $this->cfg['available_setup']['sync_recurrence'] * 3600 );
        ...
        if ( /*1 || */$current_time >= ( $first_updated_date + $recurrence ) ) {
            ...
            // here we can save wwcAmzAff_sync_cycle_stats to log before reset them bellow...
            ...
            update_option('wwcAmzAff_sync_last_updated_product', 0);
            update_option('wwcAmzAff_sync_first_updated_date', time());
            update_option('wwcAmzAff_sync_currentlist_last_product', $this->currentlist_last_product());
            update_option('wwcAmzAff_sync_currentlist_nb_products', $this->currentlist_last_product(true));
            
            $cycle_stats = get_option('wwcAmzAff_sync_cycle_stats', array());
            $cycle_stats = array_merge($cycle_stats, array(
                'start_time'        => '',
                'end_time'          => '',
            ));
            update_option('wwcAmzAff_sync_cycle_stats', $cycle_stats);
    ...
    public function cron_small_bulk( $pms, $return='die' ) {
        ...
        $currentlist_last_product = (int) get_option('wwcAmzAff_sync_currentlist_last_product', 0);
    ...
    private function save_log() {
        ...
        foreach ( (array) $res as $opt_name => $opt ) {
            if ( in_array($opt_name, array('wwcAmzAff_sync_prod_notfound')) ) {
                continue 1;
            }
    ...
    private function extract_products_by_request_size( $products=array() )
    {
        ...
        if(1){
            $last_updated_product = get_option('wwcAmzAff_sync_last_updated_product', true);
    ...
    public function updateTheTask()
    {
        ...
        // add new task
        wp_schedule_event( $timestamp, $recc, 'wwcAmzAff_SyncProducts_event');
    ...
    {
        wp_clear_scheduled_hook( 'wwcAmzAff_SyncProducts_event' );
    } 
    */
...
// reload Cronjob
//add_action('wp_ajax_wwcAmzAffSyncUpdate', 'wwcAmzAffSyncUpdate');
/*function wwcAmzAffSyncUpdate() {
	global $wwcAmzAff;
...
//add_action('wwcAmzAff_SyncProducts_event', 'wwcAmzAff_SyncProducts_event');
/*function wwcAmzAff_SyncProducts_event() {
	global $wwcAmzAff;
