...
wwcAmzAffTailSyncMonitor = (function ($) {
    "use strict";
	...
	(function init() {
		...
		$(document).ready(function(){
			...
			form_settings = 'form.wwcAmzAff-sync-settings';
            settings_wrapp = '.wwcAmzAff-sync-info';
            pagination_wrapp = '.wwcAmzAff-sync-info .wwcAmzAff-sync-pagination';
			module = mainsync.data('module');
			
            syncall_msg = maincontainer.find('#wwcAmzAff-content-area .wwcAmzAff-panel-header span.wwcAmzAff-panel-sync-all');
	...
	function loadProducts( callback ) {
        ...
        data.push({name: 'action', value: 'wwcAmzAffSyncAjax'});
        ...
        var paged = mainsync.find('.wwcAmzAff-sync-pagination.wwcAmzAff-sync-top .paging-input input.current-page').val();
        ...
        jQuery.post(ajaxurl, data, function(response) {
            ...
            if( response.status == 'valid' ){
                ...
                if ( misc.hasOwnProperty(response, 'estimate') ) {
                    mainsync.find('.wwcAmzAff-sync-filters > span span.count').html( response.estimate.nb );
                    mainsync.find('.wwcAmzAff-sync-filters > span span.countv').html( response.estimate.nbv );
                } else {
                    mainsync.find('.wwcAmzAff-sync-filters > span span.count').html( response.nb );
                    mainsync.find('.wwcAmzAff-sync-filters > span span.countv').html( response.nbv );
                }
	...
	function syncProduct( row, callback )
	{
	    ...
	    if ( $.isFunction(callback) && syncall_status == 0 ) {
            ...
            loading.find('.wwcAmzAff-loading-text').html( lang.loading );
            loading.hide();
		...
		jQuery.post(ajaxurl, data, function(response) {
			...
			if( response.status == 'valid' ){
			    var td = row.find("td");
				td.eq(3).find('> a > span').html( response.sync_hits );
				if ( response.sync_last_status != '' ) {
				    //td.eq(3).find('> i').remove();
                    //td.eq(3).append( response.sync_last_status );
                    td.eq(3).find('> a').prop( 'title', response.sync_last_status );
                }
				td.eq(4).html( response.sync_last_date );
    ...
    function goto_next( next, pms ) {
        var $parent = synctable,
            $nextel = typeof next != 'undefined' ? next : $parent.find('.wwcAmzAff-sync-table-row.wz-next-sync');
	...
	function reload_products() {
	    ...
	    mainsync.find('.wwcAmzAff-sync-filters > span.right strong').html( reload_countdown );
    ...
    function get_sync_stop_reload( action ) {
        if ( typeof action != 'undefined' ) {
            if ( action == 'set_interval' ) {
                if ( mainsync.find('.wwcAmzAff-sync-filters > span.right strong').length > 0 ) {
                    mainsync.find('.wwcAmzAff-sync-filters > span.right strong').html( reload_interval );
                }
            } else if ( action == 'stop' ) {
                mainsync.find('.wwcAmzAff-sync-filters > span.right input#sync_stop_reload').prop('checked', true);
            }
        }
        var sync_stop_reload = mainsync.find('.wwcAmzAff-sync-filters > span.right input#sync_stop_reload'),
    ...
    function open_product_variations( row, status, callback ) {
        ...
        var make_request = function( action, params, callback ){
            ...
            if ( typeof callback == 'undefined' ) loading.show();
            ...
            jQuery.post(ajaxurl, data, function(response) {
                ...
                if ( typeof callback == 'undefined' ) loading.hide();
    ...
    function open_product_variations_( row, status, callback ) {
        ...
        //var childs = synctable.find('tr.wwcAmzAff-sync-table-row.wz-variation').filter(function(i) {
    ...
    function set_rows_limits() {
        rows_nb = synctable.find('> table > tbody > tr.wwcAmzAff-sync-table-row').length;
        row_first = synctable.find('> table > tbody > tr.wwcAmzAff-sync-table-row:first');
        row_last = synctable.find('> table > tbody > tr.wwcAmzAff-sync-table-row:last');
		...
		if( status == 'show' ){
			if( row.size() > 0 ){
				if( row.find('.wwcAmzAff-row-loading-marker').size() == 0 ){
					var row_loading_box = $('<div class="wwcAmzAff-row-loading-marker"><div class="wwcAmzAff-row-loading"><div class="wwcAmzAff-meter wwcAmzAff-animate" style="width:30%; margin: 22px 0px 0px 30%;"><span style="width:100%"></span></div></div></div>')
    ...
    function triggers()
    {
        ...
        // load products
        maincontainer.on('click', '.wwcAmzAff-sync-filters span.right button.load_prods', function(e){
            e.preventDefault();
        ...
        maincontainer.on('click', '.wwcAmzAff-sync-filters span.right label, .wwcAmzAff-sync-filters span.right input', function(e){
            //e.preventDefault();

            var that = $(this), elType = that.prop('tagName').toUpperCase();
            if ( elType == 'LABEL' ) {
                maincontainer.find('.wwcAmzAff-sync-filters span.right input').trigger('click');
                return false;
            }
            auto_reload();
        });
        ...
        // sync all products
        maincontainer.on('click', '.wwcAmzAff-sync-filters span.right button.sync-all', function(e){
            e.preventDefault();

            var rows = synctable.find('> table > tbody > tr.wwcAmzAff-sync-table-row');
            ...
            loading.find('.wwcAmzAff-loading-text').html( '<strong>' + lang.sync_now_inwork + '</strong>' + '<button>' + lang.sync_now_stop_btn + '</button>' );
            loading.show();
        ...
        // stop sync all
        loading.on('click', '.wwcAmzAff-loading-text button', function(e){
            e.preventDefault();
        ...
        var make_request = function( action, params, callback ){
            ...
            if ( misc.hasOwnProperty(params, 'paged') ) {
                var totalPages_ = mainsync.find('.wwcAmzAff-sync-pagination.wwcAmzAff-sync-top .paging-input .total-pages'),
        ...
        mainsync.find( settings_wrapp ).on('change', 'select[name=wwcAmzAff-post-per-page]', function(e){
            e.preventDefault();
