...
wwcAmzAffPriceSelect = (function($) {
	"use strict";

	// public
	var debug_level = 0;
	var loading = $('<div id="wwcAmzAff-ajaxLoadingBox" class="wwcAmzAff-panel-widget">loading</div>'); // append loading
	var loading_auto = $('<div id="wwcAmzAff-ajaxLoadingBox-auto" class="wwcAmzAff-panel-widget"><div>loading</div></div>');
	var page = '';
	var product_type = '';
	...
	(function init() {
		...
		$(document).ready(function() {
			...
			priceFieldPrefix = ( page == 'details' ? '' : '_wwcAmzAff');
	...
	function is_prod_amazon( post_id ) {
		...
		if ( $hiddenWrapper.length > 0 ) {
			var is = $hiddenWrapper.find('input.wwcAmzAff-price-isprodamz').val();
	...
	function get_current_wrapper( post_id, type ) {
		...
		switch (type) {
			case 'hidden':
				wrapper = '.wwcAmzAffPriceSelectHidden';
				break;
				
			case 'wrapper':
				wrapper = '.wwcAmzAffPriceSelectWrapper';
				break;
				
			case 'wrapper-btn':
				wrapper = '.wwcAmzAff-priceselect-wrapper';
				break;
				
			case 'buttons':
				wrapper = '.wwcAmzAffPriceSelectButtons';
				break;
		}
	...
	function choose_price_default( post_id ) {
		var $wrapper = get_current_wrapper( post_id, 'wrapper' ),
   		...
   		for (var pt in priceType) {
   			var $price = $wrapper.find('input.wwcAmzAff-price-' + pt).prop('checked', false);
   			var _current = $hiddenWrapper.find('input.wwcAmzAff-price-' + pt + '-current').val().trim(),
   				_selected = $hiddenWrapper.find('input.wwcAmzAff-price-' + pt + '-selected').val().trim(),
   				_ancestry = $hiddenWrapper.find('input.wwcAmzAff-price-' + pt + '-ancestry').val().trim();
   			...
   			if ( _current == 'selected' && _ancestry != '' && _selected != ''  ) {
   				...
   				if ( _ancestry.length > 0 ) {
   					...
   					for (var key in _ancestry) {
   						_ancestryCss.push( 'ul.wwcAmzAffPriceSelect-Ancestry-' + _ancestry[key] );
   					}
   					_ancestryCss = _ancestryCss.join(' ');

   					var $_inputWrapp = $wrapper.find(_ancestryCss + ' ul.wwcAmzAff-priceselect-price span');
   					if ( $_inputWrapp.length > 0 ) {
   						$_inputWrapp.find('input.wwcAmzAff-price-' + pt).prop('checked', true);
   					}
   				}
   			}
   		}
		...
		var post_id = 0;
   		...
   		if ( page == 'details' ) {
			...
			if ( product_type == 'variable' ) {
   			...
   			//wwcAmzAff_price
   			$('td.wwcAmzAff_product_info').each(function(i) {
   				var that = $(this),
   					parent = that.parent('tr'),
   					post_id = parent.prop('id').replace('post-', '');
   					
				var $regular = parent.find('._wwcAmzAff_regular_price_field #_wwcAmzAff_regular_price'),
					$sale = parent.find('._wwcAmzAff_sale_price_field #_wwcAmzAff_sale_price');
	...
	function woo_buttons_add( post_id, $regular, $sale ) {
		...
		var btn_html = '\
		<div class="wwcAmzAff-priceselect-wrapper" data-post_id="' + post_id + '" data-pricetype="{pricetype}">\
			<a href="#" class="wwcAmzAff-price-btn-selected button button-primary button-large" data-btn="selected">select</a>\
			<a href="#" class="wwcAmzAff-price-btn-auto button button-secondary button-large" data-btn="auto">auto</a>\
		</div>\
		';
		
		/*
		var hidden_html = '';
		hidden_html += '<div class="wwcAmzAff-priceselect-hidden" data-post_id="' + post_id + '">';
		for (var pt in priceType) {
			for (var pt2 in saveMetas) {
				hidden_html += '<input type="hidden" class="wwcAmzAff-price-' + pt + '-' + pt2 + '" name="wwcAmzAff-price[' + post_id + '][' + pt + '][' + pt2 + ']" />';
			}
		}
		hidden_html += '</div>';
	...
	function btn_is_active( post_id, hiddenWrapper, wrapperBtn) {
		...
		for (var pt in priceType) {
			...
			var _current = $hiddenWrapper.find('input.wwcAmzAff-price-' + pt + '-current').val().trim();
			$btnWrapper.find('[class^="wwcAmzAff-price-btn-"]').removeClass('active');
			if ( _current != '' ) {
				$btnWrapper.find('.wwcAmzAff-price-btn-' + _current).addClass('active');
			}
		}
	}
	...
	function woo_buttons_triggers() {
    	$(document.body).on('click', ".wwcAmzAff-priceselect-wrapper > a", function(e){
    		e.preventDefault();
    		
    		var that = $(this),
    			btn = that.data('btn'),
    			$wrapperBtn = that.parents('.wwcAmzAff-priceselect-wrapper'),
    		...
    		// auto choose price
				...
				if ( $hiddenWrapper.length > 0 ) {
		    		for (var pt in priceType) {
		    			...
		    			var $price = $hiddenWrapper.find('input.wwcAmzAff-price-' + pt + '-auto');
		    			var $price_woo = get_field_price_woo( post_id, pt );
		
			    		if ( $price.length > 0 ) {
		    				var price = $price.val();
		    				//if ( price != '' ) {
	    						$hiddenWrapper.find('input.wwcAmzAff-price-' + pt + '-current').val( 'auto' );
	    						$hiddenWrapper.find('input.wwcAmzAff-price-' + pt + '-selected').val( '' );
	    						$hiddenWrapper.find('input.wwcAmzAff-price-' + pt + '-ancestry').val( '' );
    ...
    function triggers()
    {
    	...
    	var _clicked = false;
    	$('form#posts-filter').on('click', 'input#post-query-submit, input#search-submit, input#doaction, input#bulk_edit', function(e) {
            if (_clicked) {
    		...
    		$form.find('div.wwcAmzAffPriceSelectButtons').remove();
    		$form.find('div.wwcAmzAffPriceSelectHidden').remove();
    		$form.find('div[id^="wwcAmzAffPriceSelectInline"]').remove();
    		$form.find('input[name^="wwcAmzAffPriceSelectInline"]').remove();
    		//$form.submit();
    	...
    	$('table.wp-list-table #the-list input#bulk_edit, input#doaction').click(function(){
    		var $this = $(this), $form = $this.parents('form');
    		$form.find('.wwcAmzAffPriceSelectButtons').remove();
    		$form.find('input[name^="wwcAmzAffPriceSelectInline"]').remove();
    	});
    	
    	// Cancel Prices button
    	$('.wwcAmzAffPriceSelectWrapper .wwcAmzAffPriceSelect-buttons').on('click', '> a.cancel', function(e) {
    		e.preventDefault();
    		$('#TB_closeWindowButton').trigger('click');
    	});
    	
    	// Save Prices button
    	$('.wwcAmzAffPriceSelectWrapper .wwcAmzAffPriceSelect-buttons').on('click', '> a.save', function(e) {
    		e.preventDefault();
    		
    		var that = $(this),
    			$wrapper = that.parents('.wwcAmzAffPriceSelectWrapper'),
			...
			if ( $hiddenWrapper.length > 0 ) {
				...
				var _currentPrices = { 
					regular 	: $hiddenWrapper.find('input.wwcAmzAff-price-' + pt + '-auto').val(),
					sale 		: $hiddenWrapper.find('input.wwcAmzAff-price-' + pt + '-auto').val()
				};
				for (var pt in priceType) {
	    			var $price = $wrapper.find('input.wwcAmzAff-price-' + pt + ':checked');
	    			if ( $price.length > 0 ) {
	    		...
	    		for (var pt in priceType) {
		    		...
		    		if ( $price.length > 0 ) {
	    				var price = $price.parents('span').data('price');
	    				//if ( price != '' ) {
	    					$hiddenWrapper.find('input.wwcAmzAff-price-' + pt + '-current').val( 'selected' );
	    					$hiddenWrapper.find('input.wwcAmzAff-price-' + pt + '-selected').val( price );
	    					$hiddenWrapper.find('input.wwcAmzAff-price-' + pt + '-ancestry').val( get_ancestry($price) );
	...
	function save_prices( post_id, whatType, operation ) {
		...
		var data = {
			'action' 			: 'wwcAmzAffPriceSelectSave',
			'post_id'			: post_id,
			'whatType'			: whatType,
			'operation'			: operation,
		...
		var $hiddenWrapper = get_current_wrapper( post_id, 'hidden' );
		$hiddenWrapper.find('input[name^="wwcAmzAff-price"]').each(function(i) {
			var $this = $(this);
	...
	{
		tb_show( 'Amazon Product Choose Price', '#TB_inline?inlineId=wwcAmzAffPriceSelectInline-'+id );
		//tb_position();
		tb_resize();
	}
	...
	function ajaxLoading(post_id, operation, status)
    {
    	...
    	if( status == 'show' ){
        	$wrapper.append( loading_auto );
       		...
       		$wrapper.find('#wwcAmzAff-ajaxLoadingBox-auto').remove();
       	}
    }
