...
if(class_exists('wwcAmzAffCronjobsPanel') != true) {

    class wwcAmzAffCronjobsPanel {
        /*
        ...
        * Singleton pattern
        *
        * @return wwcAmzAffCronjobs Singleton instance
        */
        ...
        public function test_cron_spawn( $cache = true ) {
            ...
            $cached_status = get_transient( 'wwcAmzAff-cronjobs-test-ok' );
            ...
            if ( is_wp_error( $result ) ) {
                ...
                set_transient( 'wwcAmzAff-cronjobs-test-ok', 1, 3600 );
                return true;
            }
        ...
        public function show_cron_status() {
            ...
            if ( is_wp_error( $status ) ) {
                $msg = sprintf( __( 'Issue encountered when trying to spawn a call to the WP-Cron system on your website. The WP-Cron jobs on your website may not work. The issue details: %s', 'wwcAmzAff' ), '<br /><strong>' . esc_html( $status->get_error_message() ) . '</strong>' );
                $__status = false;
            } else {
                $msg = sprintf( __( 'Successfully spawn a call to the WP-Cron system on your website. The WP-Cron jobs on your website seems to work fine.', 'wwcAmzAff' ) );
            }
        ...
        <div class="wwcAmzAff-form-row">
            <div class="wwcAmzAff-message wwcAmzAff-<?php echo $__status ? 'success' : 'error'; ?>">
                <p><?php echo $msg; ?></p>
            </div>
        </div>
                <?php
        ...
        public function show_timezone() {
            ...
            $html = array();
            $html[] = sprintf( __( 'Local timezone is <strong>%s</strong>', 'wwcAmzAff' ), '<code>' . $tz . '</code>' );
            $html[] = '<span id="wwcAmzAff-utc-time">' . sprintf( __( 'UTC time is <strong>%s</strong>', 'wwcAmzAff' ), '<code>' . date_i18n( $time_format, false, true ) . '</code>' ) . '</span>';
            $html[] = '<span id="wwcAmzAff-local-time">' . sprintf( __( 'Local time is <strong>%s</strong>', 'wwcAmzAff' ), '<code>' . date_i18n( $time_format ) . '</code>' ) . '</span>';
        ...
        public function get_cron_events() {
            ...
            if ( empty( $crons ) ) {
                return array_merge($ret, array('msg' => __('You currently have no scheduled cron events.', 'wwcAmzAff')));
            }
            ...
            return array_merge($ret, array(
                'status'            => 'valid',
                'msg'               => sprintf( __('You have %s scheduled cron events.', 'wwcAmzAff'), count($events) ),
                'events'            => $events,
            ));
        }
        ...
        public function get_plugin_cron_events( $retType='return' ) {
            ...
            foreach( $plugin_crons as $cron_id => $cron ) {
                ...
                $args = empty( $event->args ) ? '<em>' . __( 'None', 'wwcAmzAff' ) . '</em>' : json_encode( $event->args );
                $next_run = empty($event->time) ? '<em>' . __( 'None', 'wwcAmzAff' ) . '</em>' : get_date_from_gmt(date('Y-m-d H:i:s', $event->time), $time_format)." (".$this->time_since($current_time, $event->time).")";
                $recurrence = empty($event->time) ? '<em>' . __( 'None', 'wwcAmzAff' ) . '</em>' :  ( $event->schedule ? $this->interval($event->interval) : __('Non-repeating', 'wwcAmzAff') );
                $status = $cron['status'];
                $duration = isset($cron['run_duration']) && $cron['run_duration'] != '' ? sprintf( __('%s seconds', 'wwcAmzAff'), $cron['run_duration'] ) : '';
                
                $buttons = '<input type="button" class="wwcAmzAff-button ' . ($cron['is_active'] == 'yes' ? 'red' : 'green') . '" value="' . ($cron['is_active'] == 'yes' ? __( 'Clear', 'wwcAmzAff' ) : __( 'Activate', 'wwcAmzAff' )) . '" style="width:122px">';
        ...
        public function printListInterface ()
        {
            ...
            $html = array();
            $html[] = '<style>#wwcAmzAff-amazonDebug { display: block } </style>';
            
            $html[] = '<script type="text/javascript" id="wwcAmzAff-cronjobs-js" src="' . ( $this->module['folder_uri'] ) . 'app.cronjobs.js" ></script>';
            $html[] = "<link rel='stylesheet' id='wwcAmzAff-cronjobs-css' href='" . ( $this->module['folder_uri'] ) . "app.cronjobs.css" . '?'.time() . "' type='text/css' media='all' />";

            ob_start();
        ?>

<?php /*<div class="wwcAmzAff-form-row">
    <div class="wwcAmzAff-form-item large">
        <span class="formNote"><?php _e('Cronjobs setted by our plugin.', 'wwcAmzAff'); ?></span>
    </div>
</div>*/ ?>
<?php
...
?>
<div class="wwcAmzAff-form-row wwcAmzAff-cj-list">
    ...
    <div id="wwcAmzAff-cj-reload">
        <input type="button" class="wwcAmzAff-button blue" value="<?php _e( 'Reload cronjobs list', 'wwcAmzAff' ); ?>" style="width:182px">
    </div>
    <table id="wwcAmzAff-cj-table">
        <thead>
            <tr>
                <th><?php _e('Hook Name', 'wwcAmzAff'); ?></th>
                <th><?php _e('Arguments', 'wwcAmzAff'); ?></th>
                <th><?php _e('Next Run', 'wwcAmzAff'); ?></th>
                <th><?php _e('Recurrence', 'wwcAmzAff'); ?></th>
                <th><?php _e('Last status', 'wwcAmzAff'); ?></th>
                <th><?php _e('Last duration', 'wwcAmzAff'); ?></th>
                <th><?php _e('Action', 'wwcAmzAff'); ?></th>
            </tr>
        </thead>
        <tbody>
            <?php 
...
<div class="wwcAmzAff-form-row wwcAmzAff-cj-help">
    <?php
...
//$wwcAmzAffCronjobsPanel = new wwcAmzAffCronjobsPanel($this->cfg, $module);
//$wwcAmzAffCronjobsPanel = wwcAmzAffCronjobsPanel::getInstance();