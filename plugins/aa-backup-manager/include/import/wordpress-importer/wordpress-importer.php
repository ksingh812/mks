...
class BKM_wp_import extends WP_Importer {
	...
	function BKM_wp_import() { /* nothing */ }
	...
	function import_start( $file ) {
		...
		$this->sidebars = $import_data['sidebars'];
	...
	function import_end() {
		...
		echo '<a href="' . ( admin_url( 'admin.php?page=BKM' ) ) . '" class="BKM_iw-status-notice">Click here for reload.</a>';
		
		// do some extra work:
		
		// set menu to menu positions
		$taxonomies = array(
		    'nav_menu',
		);
		...
		if( count($terms) > 0 ){
			foreach ($terms as $term) {
				if( $term->slug == 'top-menu' ){
					$top_menu = $term->term_id;
				}
				
				if( $term->slug == 'main-menu' ){
					$main_menu = $term->term_id;
				}
			}
		}
		...
		update_option( 'theme_mods_kingdom', $_arr );
		
		// find home page
		$homepage = $wpdb->get_var("SELECT ID FROM {$wpdb->posts} WHERE post_title = 'Homepage'");
		if( (int)$homepage > 0 ){
			// set the homepage as home page
...
<?php
	...
	private function available_widgets() 
	{
		...
		// add own widgets
		foreach ( $this->widgets as $sidebar_id => $widgets ) {
			if( count($widgets) > 0 ){
				foreach( $widgets as $key => $value ){
					if (strpos( $key, 'kingdom' ) !== false) {
						// force activate the widgets
						$__ = str_replace("-0", "", $key);
						$widget_id = explode( "-", $key ); 
						array_pop( $widget_id );
						
						$widget_id = $widget_id[0] . "_widget_" . $widget_id[1];
						
			
						$widget_controls[$__] = array(
							'id_base' => $__,
							'name' => $widget_id
						);
						
						update_option( $widget_id, 'true' );
					}
				}
			}
		}
	...
	public function process_widgets() 
	{
		...
		$this->widgets['wp_inactive_widgets'] = array();
		$this->widgets['orphaned_widgets_1'] = array();
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				if ( ! $fail ) {
					...
					$__widgets = $sidebars_widgets[$use_sidebar_id];
					$__new_widgets = array();
					foreach ($__widgets as $_widget) {
						if( !in_array( $_widget, $__new_widgets )){
							$__new_widgets[] = substr( $_widget, 0, -2) . '-0';
						}  
					}
					
					$sidebars_widgets[$use_sidebar_id] = $__new_widgets;
				...
				$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = isset($widget->title) ? $widget->title : __( 'No Title', 'dnh' ); // show "No Title" if widget instance is untitled
	...
	public function process_sidebars() 
	{
		if ( empty( $this->sidebars ) )
			return;

		if( count($this->sidebars) > 0 ){
			if( isset($this->sidebars['sidebars']) && count(isset($this->sidebars['sidebars'])) > 0 ){
				update_option( "kingdom_dynamic_sidebars", $this->sidebars['sidebars'] );
			}
			
			if( isset($this->sidebars['sidebars_widgets']) && count(isset($this->sidebars['sidebars_widgets'])) > 0 ){
				update_option( "sidebars_widgets", $this->sidebars['sidebars_widgets'] );
			}
			
			if( isset($this->sidebars['sidebars_per_sections']) && count(isset($this->sidebars['sidebars_per_sections'])) > 0 ){
				update_option( "kingdom_sidebars_per_sections", $this->sidebars['sidebars_per_sections'] );
			}
		}
		...
		foreach ( $this->posts as $post ) {
			...
			if ( ! empty( $post['terms'] ) ) {
				...
				foreach ( $post['terms'] as $term ) {
					...
					if ( ! $term_id ) {
						// if is woocommerce attributes
						if( preg_match("/pa_/i", $term['domain'])){
							$attribute_taxonomies = $wpdb->get_var("SELECT * FROM {$wpdb->prefix}woocommerce_attribute_taxonomies WHERE attribute_name = '".esc_sql($term['domain'])."'");
							
							if( trim($attribute_taxonomies) == "" ){
								$wpdb->insert(
					                $wpdb->prefix . 'woocommerce_attribute_taxonomies', array(
					                	'attribute_label' => $term['domain'],
					                	'attribute_name' => $term['domain'],
				...
				foreach ( $post['postmeta'] as $meta ) {
					...
					if ( $key ) {                                        
						...
						if ( ! $value )
                            ...
                            if ( $value === false )
                                $value = BKM_manager()->mb_unserialize( $meta['value'] );
	...
	function process_menu_item( $item ) {
		...
		// check if this menu already exist into same menu
		$menu_exists = $wpdb->get_row("SELECT * FROM $wpdb->posts WHERE menu_order='" . ( $item["menu_order"] ) . "' ");
		if( count($menu_exists) > 0 ){
			
			$db_item_terms = wp_get_post_terms( $menu_exists->ID, 'nav_menu' ); 
			if( $db_item_terms && count($db_item_terms) > 0 ){
				
				foreach ($db_item_terms as $db_term) {
					if( count($item['terms']) > 0){
						foreach ($item['terms'] as $term) { 
							if( $db_term->slug == $term['slug'] ){
								return;
							}
						}
					}
		...
		if ( isset($item['terms']) ) {
			...
			foreach ( $item['terms'] as $term ) {
				...
				// check if term
				$__term = term_exists( $term['name'], 'nav_menu' );
				if( (int) $__term == 0 ){
					wp_insert_term(
						$term['name'],
						'nav_menu'
		...
		if( (int) $id > 0 ){
			foreach ( $item['postmeta'] as $meta ){
				add_post_meta( $id, $meta['key'], $meta['value'] );
			}
		}
	...
	function process_attachment( $post, $url ) {
		...
		$local_file = false;
		// first time, check if file is local
		$file_relative_path = @end( @explode("uploads/", $url ) );
		$upload_dir = wp_upload_dir();
		if( is_file( $upload_dir['basedir'] . '/' . $file_relative_path )){
			$local_file = true;
			
			$upload = array(
				'file' => $upload_dir['basedir'] . '/' . $file_relative_path,
				'url' => $upload_dir['baseurl'] . '/' . $file_relative_path,
				'error' => false
			);
		}
